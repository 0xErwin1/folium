...
class PdfPageRenderer(
    ...
    override fun render(
        ...
        cache.acquire(key)?.let { return candidate(it) }
        abortIfCancelled(cancellationSignal)

        reportAspect(request.pageIndex)
        rasterizeInto(key, request, cancellationSignal)

        return cache.acquire(key)?.let { candidate(it) }
            ?: throw PdfException(PdfFailure.Resource(retryable = true))
    }

    private fun candidate(borrow: com.folium.reader.core.pdf.CachedPage<RenderedPage>) =
        RenderCandidate(BorrowedPage(borrow)) { it.release() }
    ...
    /**
     * Ownership of the raster passes to [cache] the moment it is handed over, whether or not the
     * cache keeps it — a raster too large for the budget, or crowded out by borrows held elsewhere,
     * is released by the cache rather than returned. Nothing here touches it afterwards, and the
     * borrow the caller ends up with is always taken back out of the cache rather than kept from
     * this side of the handover.
     *
     * The cache's own release of this candidate — on eviction, invalidation, trim or [cache.clear] —
     * deliberately does not recycle [RenderedPage.bitmap]. Once a raster has been shared into the
     * cache, [ReaderScreen] can be holding the derived `ImageBitmap` in a display list a frame away
     * from actually being drawn, on the render thread, independently of when the last borrow drops
     * or the cache decides to evict — so nothing on this side of the handover can prove the bitmap is
     * safe to recycle synchronously. It is left for the garbage collector instead: since API 26 a
     * [android.graphics.Bitmap]'s native pixel storage is reclaimed with the object, so what
     * [cache]'s byte budget bounds is how much is *retained*, not how much is allocated. This is
     * different from the cancellation branch just above, whose bitmap was never shared anywhere and
     * is safe to recycle immediately.
     */
    private fun rasterizeInto(
        key: PageCacheKey,
        request: ViewportRenderRequest,
        cancellationSignal: CancellationSignal
    ) {
        ...
        cache.put(key, RenderCandidate(page) {}, page.byteCount)
    }
